...
class TenantRepository(Protocol):
    ...
    async def get_by_platform_org_id(self, platform_org_id: UUID) -> Tenant | None:
        """Deliberately no tenant_id parameter — resolving which tenant a
        platform Organization maps to has to happen before any tenant
        context is bound, the same RLS-exempt shape as get_by_subdomain
        (tenants itself carries no tenant_id to scope by)."""
        ...
