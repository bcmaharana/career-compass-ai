...
class PlatformAdminGrant:
    ...
    tenant_id: UUID
    user_id: UUID
    #: Snapshot of the granted user's email/name at grant time, so the
    #: admins list can be rendered without a cross-tenant users lookup
    #: (RLS would block that without already knowing this exact
    #: tenant_id). Refreshed whenever the grant itself is updated.
    email: str
    full_name: str
