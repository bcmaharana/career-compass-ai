...
import { Crosshair, LogOut, Menu, Settings, UserCircle } from "lucide-react";
...
interface RightNavProps {
  onLogout: () => void;
  /** Mirrors Left Nav's leftNavCollapsed (AppShell.tsx) — a persistent
   * width toggle, not a drawer. Collapsed shows icon-only content
   * (profile photo, Settings/sub-nav icons) at --shell-icon-nav-w;
   * expanded shows full labels/text at --shell-right-nav-w. */
  isCollapsed: boolean;
  /** Flips isCollapsed — wired to the ☰ button in this rail's own top
   * box, visible at every screen width (same as Left Nav's). */
  onToggle: () => void;
}
 ...
 *
 * Three sections, mirroring Left Nav's skeleton: a top box (☰ toggle
 * above the profile photo/identity, both end boxes sized to their own
 * content rather than sharing --shell-side-footer-h, same reasoning as
 * Left Nav's top box); a per-page contextual slot in the middle — the
 * Career Profile page's target-roles widget (Part 2.5,
 * TargetRolesWidget.tsx, hidden while collapsed since it has no
 * icon-only form of its own), the Settings sub-nav (SETTINGS_NAV_ITEMS,
 * reduced to icon-only + a left-opening Tooltip while collapsed,
 * exactly like Left Nav's own items) under /settings/*, or a plain
 * structural placeholder otherwise, all matched by route via
 * matchNavItem/isSettingsRoute; and Settings + Sign out pinned to the
 * bottom box, also icon-only + tooltipped while collapsed.
 *
    ...
    >
      <div
        className={cn(
          ...
          // profile icon + ☰ stacked vertically on the left (same gap-1
          // rhythm as Left Nav's own logo/☰ stack, so both rails' top
          // boxes land on the same height), greeting/name/email/date as
          // a text block to their right, top-aligned to the profile
          // icon via this row's own items-start — not centered, since a
          // centered text block would drift below the icon column's top
          // edge once the icon column is shorter than the text block.
      ...
      >
        ...
        {!isCollapsed && (
          <Link to="/profile" className="min-w-0 flex-1 hover:opacity-80">
            <p
